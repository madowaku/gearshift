...
現在は **M1: Analyzer** です。タスク文または13件のfixtureを選ぶと、決定論的ルールエンジンが危険信号、推奨ギア、リスク、検証、シフトアップ条件を一画面に表示します。
...
M1を `feat/m1-rule-engine` で開発中です。M0は `v0.1.0-m0` として退避済みです。公開・課金・外部送信を行わず、解析データはブラウザ内で扱います。
